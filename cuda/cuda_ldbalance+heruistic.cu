...
__device__ bool extermination_heur(int iteration, int* ext_stack) {
	int total = 0;
	if(iteration < ext_iteration) return false;
	for(int i = 0; i < ext_iteration; i++) total += ext_stack[i];
	...
	while(1) {
		...
		while(flag) {
			...
			if( signal_recved != -1 && signal_recved < thread_index) {
				// return;				
				if(extermination_heur(iteration, ext_stack)) return;
			...
			if(next_node == -1) {
				...
				if(cuda_colormap[next_node] == -1) {
					...
					if(target_color < thread_index) {
						...
						ext_stack[(iteration++)%ext_iteration] = tree_nodes;
						tree_nodes = 0;
